...
ST LRESULT CALLBACK Desk_WndProc (HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	int n;
	static int bp; // Buttons pressed down.
...
//===========================================================================
static const char mk_mods [3] = { MK_ALT, MK_SHIFT, MK_CONTROL };
static const char modkey_strings[3][6] = { "Alt", "Shift", "Ctrl" };

unsigned get_modkeys(void)
{
	static const short vk_codes[3] = { VK_MENU, VK_SHIFT, VK_CONTROL };
	unsigned modkey = 0;
	int i = 0;
	do if (0x8000 & GetAsyncKeyState(vk_codes[i])) modkey |= mk_mods[i];
	while (++i < 3);
	return modkey;
}

/*
	Modified by Noccy to support LeftRight and RightLeft "meta"-buttons. Press the left button while holding
	the right one for the RightLeft button and so on. Should work with modifiers.
*/

bool Desk_mousebutton_event(LPARAM button, int switches)
{
	...
	int i = 0;
	do if (mk_mods[i] & modkey) strcat(rc_key, modkey_strings[i]);
	while (++i < 3);

	if (button >= 10) return false;
	sprintf(strchr(rc_key, 0), "%sClick:", button_strings[button]);

	// BBMessageBox(MB_OK,"Looking for %s",rc_key);
