...
extern "C" {
	...
	// ------------------------------------
	// Write Settings
	API_EXPORT void WriteBool(LPCSTR filePointer, LPCSTR keyword, bool value);
	API_EXPORT void WriteInt(LPCSTR filePointer, LPCSTR keyword, int value);
	API_EXPORT void WriteString(LPCSTR filePointer, LPCSTR keyword, LPCSTR value);
	API_EXPORT void WriteColor(LPCSTR filePointer, LPCSTR keyword, COLORREF value);

	// ------------------------------------
	// Delete Setting (wildcards supported)

	API_EXPORT int DeleteSetting(LPCSTR fname, LPCSTR keyword);
	// Rename Setting (wildcards supported) e.g. RenameSetting("file.rc", "bbtest.*.setting:", "bbtest.*.renamed:");
	API_EXPORT int RenameSetting(LPCSTR fname, LPCSTR old_keyword, LPCSTR new_keyword);
	...
	API_EXPORT void GetMonitorRect(void *point_or_hwnd, RECT *r, int flags);
	...
	API_EXPORT void Log(LPCSTR variable, LPCSTR description);
	...
	API_EXPORT void ReplaceShellFolders(LPSTR string);
	...
	API_EXPORT Menu *MakeNamedMenu(LPCSTR HeaderText, LPCSTR Id, bool forceshow);
	...
	API_EXPORT void DelMenu(Menu *PluginMenu);
	...
	// get the size
	API_EXPORT int GetTaskListSize();
	...
	API_EXPORT int GetTaskInfo(HWND, struct taskinfo *tlp);

	// set workspace and position for window (i.e. move task-windows over workspaces)
	API_EXPORT int SetTaskInfo(HWND, struct taskinfo *tlp);
