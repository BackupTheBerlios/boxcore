...
const char szVersion     [] = "bbLeanSkin 1.16 (BBClean SVN)";
...
void readSettings(void)
{
	...
	strncpy(mSkin.button_string, ReadString(rcpath, "bbleanskin.titlebar.buttons:", "74-321"), 15);

	// button pics
	read_buttons();

	mSkin.iconSize = ReadInt(rcpath, "bbleanskin.titlebar.icon.size:", -1);
	mSkin.iconSat = ReadInt(rcpath, "bbleanskin.titlebar.icon.saturation:", 0);
	mSkin.iconHue = ReadInt(rcpath, "bbleanskin.titlebar.icon.hue:", 60);
	...
	/// Noccy: Transparent dragging opacity
	mSkin.dragTransparency = ReadInt(rcpath, "bbleanskin.windows.dragTransparency:", 0);
	if (mSkin.dragTransparency < 0) mSkin.dragTransparency = 0;
	if (mSkin.dragTransparency > 100) mSkin.dragTransparency = 100;
...
void readStyle(void)
{
	...
	for (pG = &mSkin.windowTitleFocus; pG <= &mSkin.windowButtonUnfocus; ++pG, ++s)
	{
		...
		pG->bevelstyle = pSI->bevelstyle;
		pG->bevelposition = pSI->bevelposition;
		pG->type = pSI->type;
		pG->parentRelative = pSI->parentRelative;
		pG->interlaced = pSI->interlaced;
		pG->Color = pSI->Color;
		pG->ColorTo = pSI->ColorTo;
		pG->TextColor = pSI->TextColor;
	...
	if (newMetrics || (mSkin.windowLabelFocus.validated & VALID_MARGIN))
	{
		...
		if (lfh < mSkin.iconSize - 2) lfh = mSkin.iconSize - 2;
		else if (-1 == mSkin.iconSize) mSkin.iconSize = lfh + 2;
		DeleteObject(hf);
	...
	else
	{
		...
		else labelH = imax(mSkin.iconSize, mSkin.windowFont.Height + 2);
	}
...
void setSysClr(int f)
{
	...
	int ncolors = strcmp(GetOSInfo(), "Windows 95") ? NCOLORS : NCOLORS - 2;
	...
	if (RESTORE_3DC == f)
	{
		SetSysColors(ncolors - 2, C_ID + 2, C_SAVE + 2);
		if (changed)
		{
			SetSysColors(2, C_ID, C_SAVE);
			changed = false;
		}
		return;
	}
	...
	if (APPLY_3DC == f)
	{
		...
		if (B_SOLID == pGI->type)
			C_CR[6] = C_CR[0] = pGI->Color;
		else
		{
			C_CR[6] = pGI->ColorTo;
			C_CR[0] = mixcolors(pGI->Color, pGI->ColorTo);
		}
		...
		C_CR[2] = mSkin.windowLabelFocus.TextColor;
		C_CR[3] = mSkin.windowLabelUnfocus.TextColor;

		if (setTTColor)
		{
			C_CR[1] = mSkin.windowLabelFocus.TextColor;
			SetSysColors(ncolors, C_ID, C_CR);
			changed = true;
		}
		else
			SetSysColors(ncolors - 2, C_ID + 2, C_CR + 2);
