...
int window_startup()
{
	...
	if (!RegisterClass(&wc))
	{
		if (!plugin_suppresserrors) MessageBox(plugin_hwnd_blackbox, "Error registering window class", szVersion, MB_OK | MB_ICONERROR | MB_TOPMOST);
		return 1;
	}
...
void window_menu_context(Menu *m, control *c)
{
	...
	*/
	...
	temp = !w->is_visible;
	make_menuitem_bol(m, "Visible", config_getfull_control_setwindowprop_b(c, szWPisvisible, &temp), !temp);
	...
	if (!c->parentptr && (!w->useslit || NULL == plugin_hwnd_slit))
	{
		...
		temp = (w->makeinvisible == MAKEINVISIBLE_NEVER);
		make_menuitem_bol(submenu, "Never", config_getfull_control_setwindowprop_c(c, szWPmakeinvisible, szWPmakeinvisible_never), temp);
		temp = (w->makeinvisible == MAKEINVISIBLE_WINLOSEFOCUS);
		make_menuitem_bol(submenu, "When control loses focus", config_getfull_control_setwindowprop_c(c, szWPmakeinvisible, szWPmakeinvisible_winblur), temp);
		temp = (w->makeinvisible == MAKEINVISIBLE_BBLOSEFOCUS);
		make_menuitem_bol(submenu, "When BlackBox loses focus", config_getfull_control_setwindowprop_c(c, szWPmakeinvisible, szWPmakeinvisible_bbblur), temp);
		make_submenu_item(m, "Make Invisible", submenu);


		make_menuitem_nop(m, NULL);
		temp = !w->autohide;
		make_menuitem_bol(m, "AutoHide", config_getfull_control_setwindowprop_b(c, szWPautohide, &temp), !temp);
		temp = !w->is_ontop;
		make_menuitem_bol(m, "Always On Top", config_getfull_control_setwindowprop_b(c, szWPisontop, &temp), !temp);
		temp = !w->is_toggledwithplugins;
		make_menuitem_bol(m, "Toggle With Plugins", config_getfull_control_setwindowprop_b(c, szWPistoggledwithplugins, &temp), !temp);
		temp = !w->is_onallworkspaces;
		make_menuitem_bol(m, "On All Workspaces", config_getfull_control_setwindowprop_b(c, szWPisonallworkspaces, &temp), !temp);

		if (plugin_using_modern_os)
		{
			make_menuitem_nop(m, NULL);
			temp = !w->is_transparent;
			make_menuitem_bol(m, "Transparent", config_getfull_control_setwindowprop_b(c, szWPistransparent, &temp), !temp);
...
void window_save_control(control *c)
{
	...
	if (w->has_custom_style) //The redundance in style specification allows for a possible "Disable custom styles" option, which could come in handy for some.
	{
		for (int i = 0; i < STYLE_COLOR_PROPERTY_COUNT; ++i) //This part would be a likely candidate to be exported to another function.
		{
			...
			{
			case 0:
				colorval = w->styleptr->Color;
				break;
			case 1:
				colorval = w->styleptr->ColorTo;
				break;
			case 2:
				colorval = w->styleptr->TextColor;
				break;
			}
			char color[8];
			sprintf(color,"#%06X",switch_rgb(colorval)); //the bits need to be switched around here
	...
	if (!c->parentptr)
	{
		...
		{
		case MAKEINVISIBLE_BBLOSEFOCUS:
			makeinvisiblestring = szWPmakeinvisible_bbblur;
			break;
		case MAKEINVISIBLE_WINLOSEFOCUS:
			makeinvisiblestring = szWPmakeinvisible_winblur;
			break;
		default:
			makeinvisiblestring = szWPmakeinvisible_never;
			break;
		}
						...
						if (!stricmp(tokens[4], szWPheight) && config_set_int_expr(tokens[5], &w->height))
						{
							...
							if (!stricmp(tokens[4], szWPisbordered) && config_set_bool(tokens[5], &w->is_bordered))
							{
								...
								if (!stricmp(tokens[4], szWPStyle) && -1 != (i = get_string_index(tokens[5], szStyleNames)))
								{
									...
									{
										delete w->styleptr;    //get rid of previous custom style
										w->styleptr = 0;
									...
									if (-1 != (i = get_string_index(tokens[4], szStyleProperties)) )
									{
										...
										if (!errors)
										{
											...
											{
											case 0:
												w->styleptr->Color = colorval;
												break;
											case 1:
												w->styleptr->ColorTo = colorval;
												break;
											case 2:
												w->styleptr->TextColor = colorval;
												break;
											case 3:
												w->styleptr->bevelstyle = beveltype;
															...
															if (!stricmp(tokens[4], szWPisslitted) && config_set_bool(tokens[5], &w->useslit))
															{
																...
																if (!stricmp(tokens[4], szWPistoggledwithplugins) && config_set_bool(tokens[5], &w->is_toggledwithplugins))
																{
																	...
																	if (!stricmp(tokens[4], szWPistransparent) && config_set_bool(tokens[5], &w->is_transparent))
																	{
																		...
																		{
																			needs_posupdate = needs_transupdate = true;
																			w->is_autohidden = window_test_autohide(w, NULL);
																		}
																		else
