...
int agenttype_systemmonitor_startup()
{
	...
	if (window_helper_register(agenttype_systemmonitor_timerclass, &agenttype_systemmonitor_event))
	{
		...
		MessageBox(NULL, "failed on register class", "test", MB_OK);
		return 1;
	}
	...
	if (!agenttype_systemmonitor_window)
	{
		...
		MessageBox(NULL, "failed on window", "test", MB_OK);
		return 1;
	}
	...
	if (isnt2kxp)
	{
		...
		if (agenttype_systemmonitor_ntdllmodule == NULL)
		{
			...
			//Return immediately
			return 1;
			MessageBox(NULL, "failed on ntdll", "test", MB_OK);
		}
		...
		if (agenttype_systemmonitor_ntquerysysteminformation == NULL)
		{
			...
			MessageBox(NULL, "failed on get proc address", "test", MB_OK);
			return 1;
		}
		...
		status = agenttype_systemmonitor_ntquerysysteminformation(SystemBasicInformation, &basicinformation, sizeof(basicinformation), NULL);
		if (status != NO_ERROR)
		{
			//Couldn't get the number of processors
			FreeLibrary(agenttype_systemmonitor_ntdllmodule);
			MessageBox(NULL, "failed on processor", "test", MB_OK);
			return 1;
		}

		//Record the number of processors
		agenttype_systemmonitor_number_processors = basicinformation.NumberOfProcessors;
		...
		{
			FreeLibrary(agenttype_systemmonitor_ntdllmodule);
			MessageBox(NULL, "failed on number of processors", "test", MB_OK);
			return 1;
		}
	}
...
void agenttype_systemmonitor_updatevalue(int monitor_type)
{
	...
	int intvalue = 100*agenttype_systemmonitor_values[monitor_type];
