...
const char szInfoAuthor   [] = "psyci - additions by grischka/kana/ysuke/pkt-zer0";
const char szInfoRelDate  [] = "2006-05-29";
	...
	else if (tokencount == 5 && !stricmp(tokens[2], szBActionSetPluginProperty))
	{
		for (struct plugin_properties *p = plugin_properties; p->key; p++)
			if (p->data && 0 == stricmp(tokens[3], p->key))
			{
				...
				{
				case M_BOL:
					if (config_set_bool(tokens[4], (bool*)p->data)) break;
					return 1;
				case M_INT:
					if (config_set_int(tokens[4], (int*)p->data)) break;
					return 1;
				case M_STR:
					if (config_set_str(tokens[4], (char**)p->data)) break;
					return 1;
				default:
					return 1;
				}
...
void plugin_save()
{
	...
	for (struct plugin_properties *p = plugin_properties; p->key; p++)
		if (p->data) switch (p->type)
			{
			case M_BOL:
				config_write(config_get_plugin_setpluginprop_b(p->key, (bool*)p->data));
				break;
			case M_INT:
				config_write(config_get_plugin_setpluginprop_i(p->key, (const int*)p->data));
				break;
			case M_STR:
				config_write(config_get_plugin_setpluginprop_c(p->key, *(const char**)p->data));
				break;
			}
