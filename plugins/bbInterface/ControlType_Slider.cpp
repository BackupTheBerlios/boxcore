...
LRESULT controltype_slider_event(control *c, HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		if (style_draw_begin(c, d))
		{
			...
			if (STYLETYPE_TOOLBAR == c->windowptr->style)
...
void controltype_slider_menu_context(Menu *m, control *c)
{
	...
	temp = !details->reversed;
	make_menuitem_bol(m, "Values Reversed", config_getfull_control_setcontrolprop_b(c, "Reversed", &temp), !temp);
	...
	temp = false;
	make_menuitem_bol(submenu1, "Horizontal", config_getfull_control_setcontrolprop_b(c, "Vertical", &temp), !details->vertical);
	temp = true;
	make_menuitem_bol(submenu1, "Vertical", config_getfull_control_setcontrolprop_b(c, "Vertical", &temp), details->vertical);
