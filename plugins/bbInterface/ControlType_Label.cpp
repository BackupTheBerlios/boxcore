...
void controltype_label_updatesettings(controltype_label_details *details)
{
	...
	{
	case 0:
		details->settings = (DT_SINGLELINE | DT_VCENTER);
		break;
	case 1:
		details->settings = (DT_SINGLELINE | DT_TOP);
		break;
	case 2:
		details->settings = (DT_SINGLELINE | DT_BOTTOM);
		break;
	case 3:
		details->settings = (DT_WORDBREAK | DT_TOP );
		break;
	}
	switch (details->halign)
	{
	case 0:
		details->settings = details->settings | DT_CENTER;
		break;
	case 1:
		details->settings = details->settings | DT_LEFT;
		break;
	case 2:
		details->settings = details->settings | DT_RIGHT;
		break;
	}
}
...
{
	sprintf(config_masterbroam, "%s %s %s %s %s", szBBroam, szBEntityControl, szBActionPlugin, c->controlname, key);
	return config_masterbroam;
}
char *config_get_control_plugin_c(control *c, const char *key, const char * plug)
{
	sprintf(config_masterbroam, "%s %s %s %s %s \"%s\"", szBBroam, szBEntityControl, szBActionPlugin, c->controlname, key, plug);
	return config_masterbroam;
}
char *config_get_control_plugin_prop_b(control *c, const char * plug, const char *key, bool val)
{
	sprintf(config_masterbroam, "%s %s %s %s %s %s %s %s", szBBroam, szBEntityControl, szBActionPlugin, c->controlname, szBActionPluginSetProperty, plug, key, szBoolArray[val]);
	return config_masterbroam;
}
char *config_get_control_plugin_prop_ii(control *c, const char * plug, const char *key, int x, int y)
{
	sprintf(config_masterbroam, "%s %s %s %s %s %s %s %d %d", szBBroam, szBEntityControl, szBActionPlugin, c->controlname, szBActionPluginSetProperty, plug, key, x, y);
	return config_masterbroam;
}

char *config_getfull_control_plugin_s(control *c, const char *key)
{
	sprintf(config_masterbroam, "%s %s %s %s:%s %s", szBBroam, szBEntityControl, szBActionPlugin, c->moduleptr->name, c->controlname, key);
	return config_masterbroam;
}
char *config_getfull_control_plugin_c(control *c, const char *key, const char * plug)
{
	sprintf(config_masterbroam, "%s %s %s %s:%s %s \"%s\"", szBBroam, szBEntityControl, szBActionPlugin, c->moduleptr->name, c->controlname, key, plug);
	return config_masterbroam;
}
char *config_getfull_control_plugin_prop_b(control *c, const char * plug, const char *key, bool val)
{
	sprintf(config_masterbroam, "%s %s %s %s:%s %s %s %s %s", szBBroam, szBEntityControl, szBActionPlugin, c->moduleptr->name, c->controlname, szBActionPluginSetProperty, plug, key, szBoolArray[val]);
	return config_masterbroam;
}
char *config_getfull_control_plugin_prop_ii(control *c, const char * plug, const char *key, int x, int y)
{
	sprintf(config_masterbroam, "%s %s %s %s:%s %s %s %s %d %d", szBBroam, szBEntityControl, szBActionPlugin, c->moduleptr->name, c->controlname, szBActionPluginSetProperty, plug, key, x, y);
	return config_masterbroam;
}
