...
LRESULT CALLBACK message_event(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	static const int message_blackboxmessages[] = {BB_RECONFIGURE, BB_BROADCAST, BB_DESKCLICK, BB_DRAGTODESKTOP, BB_HIDEMENU, 0};
...
int varset_message(int tokencount, char *tokens[], bool from_core, module* caller)
{
	...
	{
		int i;
		const char *c = variables_get(tokens[2],NULL, caller); //Get current value
		if	(	c
				&&	config_set_int(c,&i) //Try reading it as an int
				&&	config_set_int_expr(tokens[3],&i) // Try calculating the string as an expression
		   )
			variables_set(false,tokens[2],i, caller);
		else variables_set(false, tokens[2], tokens[3], caller);
		return 0;
	}
	else
		if (tokencount == 5 && !strcmp(tokens[2], "Static") )
		{
			int i;
			const char *c = variables_get(tokens[3],NULL, caller);
			if	(	c
					&&	config_set_int(c,&i)
					&&	config_set_int_expr(tokens[4],&i)
			   )
				variables_set(true,tokens[3],i, caller);
			else variables_set(true, tokens[3], tokens[4], caller);
...
char *message_preprocess(char *buffer, module* defmodule)
{
	...
	while (NULL != (start = strchr(start, '$')) && NULL != (end = strchr(start+1, '$')))
	{
		...
		if (varlen)
		{
			...
			// Check if it refers to a property
			...
			else
			{
				...
				if (char* cnameend = strchr(expression, '.')) // Refers to a property of a control.
				{
					...
					else if (control *c = control_get(cname) )
					{
						get_property_by_name(expression, c, propname);
						replacement = expression;
					}
