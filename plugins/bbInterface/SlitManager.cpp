...
int SlitWndProc(control *c, HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
		...
		//dbg_window ((HWND) lParam, "add");
	{
		...
		p->hwnd = (HWND)lParam;
		p->hMO = (HMODULE)GetClassLongPtr(p->hwnd, GCLP_HMODULE);
		...
		*pp = p, p->next = NULL;
		...
		p->wp = (WNDPROC)SetWindowLongPtr(p->hwnd, GWLP_WNDPROC, (LONG_PTR)subclass_WndProc);
	...
	case SLIT_REMOVE:
		...
		while (*pp)
		{
			if ((*pp)->hwnd == (HWND)lParam)
			{
				...
				if (IsWindow(p->hwnd))
				{
					SetWindowLongPtr(p->hwnd, GWLP_WNDPROC, (LONG_PTR)p->wp);
