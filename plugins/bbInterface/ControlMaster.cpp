...
void control_menu_settings(Menu *m, control *c)
{
	...
	n = 0;
	sub = make_menu("Parent Control", c);
	...
	else
	{
		...
		if (c->controltypeptr->can_child && NULL == c->parentptr && false == c->windowptr->is_slitted)
		{
			...
			dolist (ln2, globalmodule.controllist)
			{
				...
				{
					char buffer[110];
					sprintf(buffer, "AttachTo %s:%s", pspec->moduleptr->name, pspec->controlname);
	...
	if (!c->parentptr)
		...
		else
		{
			...
			dolist (ln, modulelist)
			{
				...
				{
					++n;
					make_menuitem_cmd(sub, m->name, config_getfull_control_assigntomodule(c, m));
				}
			}
