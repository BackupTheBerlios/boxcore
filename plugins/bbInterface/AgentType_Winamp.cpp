...
int agenttype_winamp_create(agent *a, char *parameterstring)
{
	...
	if (code < 40001 || code > 40323) return 1;
...
int agenttype_winamppoller_create(agent *a, char *parameterstring)
{
	...
	if (commandcode == 0)
	{
		...
		if (!plugin_suppresserrors) MessageBox(NULL, tempstring, szAppName, MB_OK|MB_SYSTEMMODAL);
		return 1;
	}
...
void *agenttype_winamppoller_getdata(agent *a, int datatype)
{
	...
	case DATAFETCH_VALUE_TEXT:
		...
		{
		case WINAMP_POLLINGTYPE_TITLE:
			return agenttype_winamp_currenttitle;
			break;
		case WINAMP_POLLINGTYPE_TIME_ELAPSED:
			return agenttype_winamp_timeelapsed;
			break;
		case WINAMP_POLLINGTYPE_TIME_REMAINING:
			return agenttype_winamp_timeremaining;
			break;
		case WINAMP_POLLINGTYPE_TIME_TOTAL:
			return agenttype_winamp_timetotal;
			break;
		case WINAMP_POLLINGTYPE_BITRATE:
			return agenttype_winamp_bitrate;
			break;
		default:
			return NULL;
			break;
	...
	case DATAFETCH_VALUE_SCALE:
		return &agenttype_winamp_trackposition;
		break;

	case DATAFETCH_VALUE_BOOL:
		return &agenttype_winamp_isplaying;
		break;
	}
	return NULL;
}
