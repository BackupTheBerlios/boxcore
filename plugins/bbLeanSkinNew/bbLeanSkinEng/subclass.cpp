...
void PaintAll(struct WinInfo* WI)
{
    ...
    LONG w_style = WI->style; int pos, i;
    for (pos = i = 0; i<6; i++)
    {
        static char trans[6] = { 0,1,2,5,4,3 };
        int n = trans[i];
        ...
        if (set)
        {
            ...
            {
                if (5 == pos) rc.right = label_right - mSkin.buttonMargin;
...
int get_button (struct WinInfo *WI, int x, int y)
{
    ...
    else
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            if (3 == i) left = right - 3 * button_next + mSkin.buttonSpace;
            if (x < left) break;
...
*/
    ...
    case WM_NCLBUTTONDOWN:
        ...
        if (n >= 0 && n < 6) goto set_capture; // clicked in a button
