/*
 ...
 bbLeanSkinMod 1.16m (2006-01-07) / qwilk
	- Based on bbLeanSkin 1.16 (2005-04-27) by grischka
	- Replaced bbLean proprietary code for compatibility with xoblite/bb4win
	  (i.e. added stand-alone window style parsing code to the plugin)
	- Added bro@ms list menu support
	- Minor tweaks and changes
	- Please note that no changes were made to the engine (bbLeanSkinEng)

 bbLeanSkin 1.16 (2005-04-27) / grischka
	- Original release

 ============================================================================
*/

#include "bbLeanSkinMod.h"

//============================================================================
// info

const char szAppName     [] = "bbLeanSkinMod";
const char szVersion     [] = "bbLeanSkinMod 1.16m";
const char szInfoVersion [] = "1.16m";
const char szInfoAuthor  [] = "grischka/TheDevTeam";
const char szInfoRelDate [] = "2006-01-07";
const char szInfoLink    [] = "http://bb4win.org/";
const char szInfoEmail   [] = "";

void about_box(void)
{
	char buff[256];
	sprintf(buff, "%s   \n\n© 2003-2005 grischka@users.sourceforge.net   \n© 2003-2004/2006 The Blackbox for Windows Development Team   \n\nPartly based on ShellWM   \n© 2002 ShellWM Development Team   \nPartly based on CustomEyes   \n© 1999-2001 Didier Abderrahmane   \n", szVersion);
...
{
	/*
		// ##### NOT COMPATIBLE WITH XOBLITE/BB4WIN - BBLEAN ONLY #####
	...
	*/
	...
	// ##### COMPATIBLE WITH XOBLITE/BB4WIN/BBLEAN #####
	...
	// NOTE: bbLean seems to use SN_NEWMETRICS as a flag for bb4nix
	// 0.70 syntax (?), but the compatible style parsing engine
	// in compatibleReadStyle() handles both old and new syntax
	// (and mixes thereof) so we set the flag to "true"...
	bool newMetrics = true;
	...
	if (newMetrics || (mSkin.windowLabelFocus.validated & VALID_MARGIN))
	{
		...
		HDC hdc = CreateCompatibleDC(NULL);
		int data = 0;
		EnumFontFamilies(hdc, mSkin.windowFont.Face, (FONTENUMPROC)EnumFontFamProc, (LPARAM)&data);
		DeleteDC(hdc);
		if (!data) strcpy(mSkin.windowFont.Face, "verdana");

		HFONT hf =  CreateFont(mSkin.windowFont.Height, 0, 0, 0, mSkin.windowFont.Weight, false, false, false,	DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH|FF_DONTCARE, mSkin.windowFont.Face);

		TEXTMETRIC TXM;
		HDC temphdc = CreateCompatibleDC(NULL);
		HGDIOBJ other = SelectObject(temphdc, hf);
		GetTextMetrics(temphdc, &TXM);
		SelectObject(temphdc, other);
		DeleteDC(temphdc);
		int lfh  = TXM.tmHeight;// - TXM.tmInternalLeading - TXM.tmExternalLeading;
	...
	int tbheight =
		imax(labelH, buttonH) + 2 * (mSkin.windowTitleFocus.borderWidth + mSkin.windowTitleFocus.marginWidth);
...
/*
 ============================================================================
 The code below was partly based on the xoblite style parsing implementation
...
char borderColorAsString[MAX_LINE_LENGTH];
...
char temp1[MAX_LINE_LENGTH], temp2[MAX_LINE_LENGTH], param[MAX_LINE_LENGTH];
